...
for file in files:
    ...
    noise_dim = np.random.normal(0, 1e-5, size=data_array.shape) 
    data_array_2d = np.hstack([data_array, noise_dim])
    ...
    bounds= create_bounds(data_array)
    noise_bounds = np.array([[-1e-4, 1e-4]], dtype=np.float32)
    bounds_2d = np.vstack([bounds, noise_bounds])
    # Train the Flow on the data.
    flow = NormalizingFlow(2, bounds=bounds_2d, n_neurons=10, num_bins=4)
    history = flow.fit(data_array_2d, n_samples=100000)
    

    hist_max = max(flow.prob(data_array_2d))
