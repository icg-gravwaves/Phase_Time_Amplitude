...
[Flow Sampler](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Sampling/PTA_samples.py)
...
[Normalizing Flow Trainer](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Normalizing_Flow/ML_training.py)
...
[Normalizing Flow Stat File](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Normalizing_Flow/ml_stat.py)
...
[Log Signal-ratio](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/signal_ratio.ipynb): This takes the output of the modified sampler as well as the output of the original histogram sampler (https://github.com/gwastro/pycbc/blob/master/bin/all_sky_search/pycbc_dtphase) and plots the distribution of the signal ratio in both cases.

[Uncertainty correlations](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Uncertainty_Modelling/Uncertainties.ipynb): This simulates a bunch of GW signals, performs a matched-filter search and performs statistical tests on the uncertainities measured. This uses the helper script [Helper](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Uncertainty_Modelling/uncertainties_utilities.py).

[4det Backgrounds](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/Backgrounds.ipynb): This takes some outputs of the 4 det search and plots the backgrounds of all multi-ifo combinations. Members of LIGO should be able to run this on the Caltech cluster, otherwise you can reuqest the files and we would be happy to provide as they are too large to include here. This uses the code found in the PyCBC executable pycbc_page_fars_vs_stat so can simialrly be generated using this. (https://github.com/gwastro/pycbc/blob/master/bin/plotting/pycbc_page_fars_vs_stat)

[4det Found/Missed](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/Found_missed.ipynb): This takes some outputs of the 4 det search and plots the distributions of found and missed injections. Members of LVK should be able to run this on the Caltech cluster, otherwise you can reuqest the files and we would be happy to provide as they are too large to include here. This uses the code found in the PyCBC executable pycbc_page_foundmissed so can similarly be generated using this. (https://github.com/gwastro/pycbc/blob/master/bin/plotting/pycbc_page_foundmissed)

[File_size](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/File_size.ipynb): This compares the file sizes for the two different methodologies, to run this you will need output files for the two methodologies.

[Appendix Amplitude Distribution](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/Appendix_amplitude_dist.ipynb): This just shows the distributions of amplitudes due to our inclusions of distance. No files are required to run this.

[Appendix ML training](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/Flow_training.ipynb): This produces corner plots comparing the distribution learned by the flow to the training data. To run this you will need both the smaples the NF was trained on as well as the output file from the ML-training script.

[Cost](https://github.com/icg-gravwaves/Phase_Time_Amplitude/blob/main/Plotting/Cost.ipynb): This produces a comparison of the runtimes of the two different methodogies. The times we used from our test can be found here ([Original_2DET](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/Timing_results/timing_results_Original_2DET.csv), [Original_3DET](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/Timing_results/timing_results_Original_3DET.csv), [FLOW_2DET](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/Timing_results/timing_results_Flow_2DET.csv), [FLOW_3DET](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/Timing_results/timing_results_Flow_3DET.csv)). These tests were all completed on the same machine, running on a single CPU without other processes running in parallel.

[Recovered Injections](https://raw.githubusercontent.com/icg-gravwaves/Phase_Time_Amplitude/main/Plotting/plot_fraction_found_injs.ipynb): The plots the fraction of found injections for each method. This requires the combined HDFINJFIND files from each search, some of these will also need to be recreated for certain detector combinations by rerunning the pycbc_coinc_hdfinjfind jobs but only including the statmap files of that specific detector combination. These files are too large to include here but we are happy to share the ones we used if you request.
